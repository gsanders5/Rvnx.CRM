...
{
    public static async Task SyncUserAndEnrichClaimsAsync(this ClaimsIdentity identity, IServiceProvider services)
    {
        ...
        var userSync = services.GetRequiredService<IUserSynchronizationService>();

        string? subjectId = identity.FindFirst(ClaimTypes.NameIdentifier)?.Value
