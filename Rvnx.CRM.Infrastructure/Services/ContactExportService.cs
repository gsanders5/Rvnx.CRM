...
public class ContactExportService(IRepository repository, IVCardService vCardService) : IContactExportService
{
    ...
    public async Task<ContactExportResult> ExportToVCardAsync(Guid contactId)
    {
        ...
        contact.ContactMethods = await _repository.ListAsync<ContactMethod>(e => e.EntityId == contactId && e.EntityType == EntityTypes.Person);
        contact.SignificantDates = await _repository.ListAsync<SignificantDate>(e => e.EntityId == contactId && e.EntityType == EntityTypes.Person);
