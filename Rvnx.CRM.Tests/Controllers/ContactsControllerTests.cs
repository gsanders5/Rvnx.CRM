...
public class ContactsControllerTests
{
    public class SelfContactTests
    {
        ...
        public async Task CreateSelfGetShouldPreFillData()
        {
            ...
            selfContactMock.Setup(s => s.GetSelfContactIdAsync(It.IsAny<System.Security.Claims.ClaimsPrincipal>())).ReturnsAsync((Guid?)null);

            ContactFormDto formDto = new() { FirstName = "Test", LastName = "User", Email = "test@example.com" };
            selfContactMock.Setup(s => s.GetSelfContactFormAsync(It.IsAny<System.Security.Claims.ClaimsPrincipal>())).ReturnsAsync(formDto);
        ...
        public async Task SelfShouldRedirectToDetailsWhenSelfContactExists()
        {
            ...
            Mock<ISelfContactService> selfContactMock = new();
            selfContactMock.Setup(s => s.GetSelfContactIdAsync(It.IsAny<System.Security.Claims.ClaimsPrincipal>())).ReturnsAsync(selfContactId);
        ...
        public async Task CreateSelfPostShouldCreateContactAndLinkUser()
        {
            ...
            selfContactMock.Setup(s => s.CreateSelfContactAsync(It.IsAny<System.Security.Claims.ClaimsPrincipal>(), It.IsAny<ContactFormDto>()))
