...
namespace Rvnx.CRM.Core.Helpers
{
    public static class SocialMediaUrlNormalizer
    {
        public static string Normalize(ContactMethodType type, string value)
        {
            ...
            {
                if (type == ContactMethodType.Twitter && value.Contains("x.com", StringComparison.OrdinalIgnoreCase))
                {
                    return value.Replace("x.com", "twitter.com", StringComparison.OrdinalIgnoreCase);
                }
                return value;
            }
