...
## 2024-03-23 - Keep infrastructure dependencies out of Core domain models
**Learning:** Domain models within the `Core` project should remain completely isolated and oblivious to persistence frameworks (like Entity Framework Core) and UI presentation concerns (like ASP.NET Core MVC DataAnnotations). Mixing attributes like `[Table]`, `[ForeignKey]`, and `[Display]` directly on these models breaks clean architecture by tying the domain layer directly to specific database schemas and UI structures.
**Action:** When creating or modifying domain entities, define database mapping configurations (e.g., constraints, foreign keys, table names) using the Fluent API within the `CRMDbContext` located in the `Infrastructure` project. Ensure any UI-specific display logic or validation is relegated to specialized DTOs or ViewModels in the `Web` layer, leaving domain models pure. When removing existing `[Table("Name")]` attributes, always explicitly configure the mapping in the Fluent API (`modelBuilder.Entity<T>().ToTable("Name")`) to prevent EF Core from accidentally renaming existing tables using its default pluralization conventions.
