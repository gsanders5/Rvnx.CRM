...
## 2026-06-13 - Optimize Dictionary grouping with ToDictionary
**Learning:** Creating a `Dictionary<K, List<V>>` using `.GroupBy(x => x.Key).ToDictionary(...)` introduces unnecessary overhead due to the instantiation of intermediate `IGrouping` structures and requires redundant list iterations.
**Action:** For high-performance dictionary creation where collections of elements are grouped by key, avoid using `GroupBy().ToDictionary(...)`. Instead, pre-allocate a `Dictionary` (if count is known) and manually populate it using a `foreach` loop.
