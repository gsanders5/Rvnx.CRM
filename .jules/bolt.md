...
## $(date +%Y-%m-%d) - Optimize Activity Participant Mapping in ContactReadService
**Learning:** In C#/.NET, chaining multiple LINQ passes (e.g., `.Where(...).ToList()` followed by multiple `.Select(...).ToList()`) inside inner loops, especially when performing dictionary lookups, creates significant memory pressure and unnecessary CPU overhead due to multiple collection enumerations and redundant lookups (e.g., `ContainsKey` followed by indexer `[]`).
**Action:** When mapping nested structures like activity participants in a loop, avoid chaining LINQ `.Where` and `.Select`. Instead, perform a single `foreach` pass with pre-allocated list capacities (e.g., `new List<T>(capacity)`) and use `TryGetValue` to minimize iterations, list allocations, and redundant dictionary accesses.
