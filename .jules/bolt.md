...
## 2026-05-10 - Avoid redundant dataset iterations when creating multiple collections
**Learning:** In `DashboardService.cs`, the code was iterating over an unbounded `contacts` dataset twice: once via `.ToDictionary()` and once via a collection expression `[.. contacts.Select()]`. This pattern incurred unnecessary allocations for LINQ state machines, closures, and dynamic array resizing.
**Action:** When creating multiple collections from the same dataset, pre-allocate the target collections with their respective capacities (e.g., `new Dictionary<Guid, ContactSummary>(contacts.Count)` and `new List<Guid>(contacts.Count)`) and populate them simultaneously within a single `foreach` loop to eliminate redundant iterations and resizing overhead.
