...
## 2024-06-28 - Optimize Dictionary allocation from GroupBy
**Learning:** Creating a `Dictionary<K, List<V>>` using `.GroupBy(x => x.Key).ToDictionary(...)` introduces unnecessary overhead due to the instantiation of intermediate `IGrouping` structures and redundant iterations.
**Action:** Replace this pattern with a single-pass `foreach` loop that populates a manually pre-allocated `Dictionary` with initial capacity, provided the dataset is sufficiently large or unbounded.
