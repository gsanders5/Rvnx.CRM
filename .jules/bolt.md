...
## 2024-06-29 - Avoid GroupBy allocations during large exports
**Learning:** Creating a `Dictionary<K, List<V>>` using `.GroupBy(x => x.Key).ToDictionary(...)` inside heavy data pipelines (like `ContactExportService`) introduces unnecessary overhead due to the instantiation of intermediate `IGrouping` structures, causing extra memory pressure during large exports.
**Action:** Replace `.GroupBy().ToDictionary()` with a pre-allocated `Dictionary` populated manually using a `foreach` loop. Use `TryGetValue` and `TryAdd` to initialize and populate the lists efficiently, avoiding `IGrouping` allocations entirely.
