...
## 2026-04-18 - Avoid full entity fetch for resolving display names in Relationship Suggestions
**Learning:** `RelationshipSuggestionService` was fetching full `Contact` entities into memory via `GetByIdAsync` just to extract the `FullName` for displaying relationship suggestions. This is inefficient as `Contact` contains large string fields and potential navigation properties.
**Action:** When resolving entity display names, use `ListProjectedAsync<T, string>` to query only the necessary name components directly from the database and format them in SQL or in memory depending on the query provider capabilities.
