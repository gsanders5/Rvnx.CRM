...
## 2026-04-18 - Dictionary Pre-allocation in ContactExportService
**Learning:** `ContactExportService.LoadGroupedAsync` was chaining `.GroupBy().ToDictionary()` for bulk grouping of contact metadata. Using this LINQ pattern allocates intermediate `IGrouping` instances and triggers redundant iterations over the dataset before producing the final Dictionary.
**Action:** Replace `GroupBy().ToDictionary()` with a manual loop that checks `TryGetValue()` and adds to a pre-allocated/managed list within the Dictionary. Always document *why* the optimization exists inline (e.g., "Optimization: Manually populate to avoid allocating IGrouping") so future maintainers understand the verbose loop is intentional.
