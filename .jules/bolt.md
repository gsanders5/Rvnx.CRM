...
## 2026-05-18 - Optimize Dictionary Creation from Grouped Data
**Learning:** Using the LINQ chain `.GroupBy(keySelector).ToDictionary(g => g.Key, g => g.ToList())` allocates unnecessary intermediate `IGrouping` structures and increases garbage collection pressure, particularly on hot paths or large datasets (like contact exports).
**Action:** Replace `GroupBy().ToDictionary()` with a pre-allocated `Dictionary` and a single-pass `foreach` loop. Use `TryGetValue` to append items to existing lists or `TryAdd` to initialize new ones. Size the initial dictionary capacity accurately based on the distinct keys (e.g., `contactIds.Count`) to avoid bucket resizing overhead.
