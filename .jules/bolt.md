...
## 2026-06-18 - Optimize relationship suggestions by avoiding full entity fetches
**Learning:** `GetSuggestedRelationshipsAsync` in `RelationshipSuggestionService.cs` was executing `GetByIdAsync<Contact>` and mapping full `Contact` lists simply to resolve the `FullName` of contacts. This caused EF Core to load the entire `Contact` entity into memory, leading to unnecessary data transfer and object allocation overhead.
**Action:** Replace `GetByIdAsync` and `ListAsNoTrackingAsync` with `ListProjectedAsync` to selectively fetch and map `(c.FirstName + " " + (c.LastName ?? "")).Trim()` into `ValueTuple<Guid, string>` whenever resolving the display name of a contact during relationship suggestions.
