...
## 2026-04-19 - Avoid Micro-Optimizing Bounded Collections
**Learning:** Applying LINQ allocation optimizations (such as replacing `.Where(...).ToHashSet()` with pre-allocated HashSets and loops) to extremely small, hardcoded limit datasets (e.g., maximum 50 nodes in a graph search) provides zero measurable performance benefit and drastically degrades code readability.
**Action:** When applying memory allocation optimizations like pre-allocated dictionaries or HashSets, first verify the target dataset is potentially large or unbounded. Do not replace concise LINQ with verbose boilerplate for micro-collections.
