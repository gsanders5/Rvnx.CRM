...
## 2026-06-17 - Avoid LINQ GroupBy.ToDictionary Overhead
**Learning:** Creating a `Dictionary<K, List<V>>` using `.GroupBy(x => x.Key).ToDictionary(...)` introduces unnecessary overhead due to the instantiation of intermediate `IGrouping` structures and iterators, causing excessive memory allocations especially inside nested loops or heavy grouping operations.
**Action:** When grouping collections into a Dictionary of Lists, pre-allocate the Dictionary and populate it manually using a single `foreach` loop and `TryGetValue` to find or create the list. This avoids all LINQ grouping allocations.
