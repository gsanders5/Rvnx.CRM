...
## 2024-02-28 - Testing Repository Extension Methods with Moq
**Learning:** `IRepository.IsValidContactAsync` is an extension method and cannot be mocked directly with Moq. Attempting to do so results in compilation errors. Additionally, methods on `IRepository` that have optional parameters (like `CancellationToken`) must have `It.IsAny<CancellationToken>()` explicitly provided in the `Setup` and `Verify` calls to avoid `CS0854` errors.
**Action:** Mock the underlying repository method that the extension method calls (e.g., `CountAsync<Contact>`) instead of the extension method itself. Always supply explicit arguments for optional parameters in Moq expressions.
